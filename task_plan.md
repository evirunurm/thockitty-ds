...
- Ensure the project is set up with Node.js (v18+ recommended) and pnpm as the package manager.
...
- Install core development tools for building a modern React library:
    - Use TypeScript for type safety: `pnpm add -D typescript`.
    - Use parcel as a bundle.
    - Install React types for development: `pnpm add -D @types/react @types/react-dom`.
- Install React Aria dependencies:
    - Direct dependency: `@react-aria/button` (since we're wrapping it specifically for the Button component): `pnpm add @react-aria/button`.
...
- Add build scripts to `package.json`. Example:
    ...
    ```
- Run `pnpm run build` to generate `dist/` with ESM (.mjs), CJS (.cjs), and types (.d.ts). This makes it a consumable npm package supporting modern bundlers.
